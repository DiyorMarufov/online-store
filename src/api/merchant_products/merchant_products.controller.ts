...
export class MerchantProductsController {
  ...
  @Get('merchant')
  @ApiTags('Merchant Products')
  @ApiOperation({ summary: 'Get all products for merchant' })
  @ApiResponse({
    status: 200,
    description: 'List of merchant products returned successfully.',
  })
  @ApiResponse({
    status: 401,
  ...
  findAllForMerchant(@CurrentUser() user: UsersEntity) {
    return this.merchantProductsService.findAllForMerchant(user);
  }
